...
class BrainDQNMain:
    def save(self):
        ...
        torch.save(self.Q_net.state_dict(), 'paramsgpu.pth')
        
    def load(self):
        if os.path.exists("paramsgpu.pth"):
            print("load model param")
            self.Q_net.load_state_dict(torch.load('paramsgpu.pth'))
            self.Q_netT.load_state_dict(torch.load('paramsgpu.pth'))
