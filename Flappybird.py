...
class BrainDQNMain:
    def save(self):
        ...
        torch.save(self.Q_net.state_dict(), 'params0.pth')
        
    def load(self):
        if os.path.exists("params0.pth"):
            print("load model param")
            self.Q_net.load_state_dict(torch.load('params0.pth'))
            self.Q_netT.load_state_dict(torch.load('params0.pth'))
    ...
    def train(self): # Step 1: obtain random minibatch from replay memory
        ...
        for i in range(0, BATCH_SIZE):
            ...
            if terminal:
                ...
                y_predict=self.Q_net(state_batch_tensor).gather(1,action_batch_tensor)###根据action得到的当前预测的y值
    ...
    def getAction(self):
        ...
        QValue = self.Q_net(currentState)[0] #把黑白图喂进神经网络，得到一个长为2的数组
        action = np.zeros(self.actions) #action为数组 [0,0]
        if self.timeStep % FRAME_PER_ACTION == 0:
             if random.random() <= self.epsilon: #episilon-greedy,choose random
                 action_index = random.randrange(self.actions)
                 print("choose random action1 "+str(action_index))
                 action[action_index] = 1 #选择动作
                 #action[0]=1;
             else:
                     action_index = np.argmax(QValue.detach().numpy()) #episilon-greedy,choose greedy
                     print("choose qnet value action2 " + str(action_index)) 
                     action[action_index] = 1 #选择动作
                     #action[0]=1
        else:
